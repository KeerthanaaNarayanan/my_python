...
dummy_file = ' --g_build /logs/downloads/dummy.txt'
user = ' --g_user knarayanan'  # 2. change the user name to your name
product = ' --g_product TZ'
env = ' --g_env QA_BETA'
...
suite_name_individual = [
    ...
    'objects/content_filter/content_filter_profile_object_valid_create_category.py'
...
def main():
    ...
    for i in range(count):
        ...
        half_command = dummy_file + user + product + scmlabel + label_scm.get(db, "") + vtb.get(host, "") + env
        if swift == 'swift':
            command = 'python3 ' + str(prod_suite_path) + str(half_command)
        if swift == 'devtest':
            command = 'python3 ' + str(dev_suite_path) + str(half_command)
        print(command)
